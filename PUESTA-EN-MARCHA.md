...
| Proyecto Supabase | Usuarios, IDs, productos, inventario, pedidos y chat | Ejecutar 01-schema.sql si es nuevo; 03-security.sql si ya instalaste la versión anterior |
...
| Vercel | Servir la web por HTTPS | Importar GitHub, Vite, build npm run build, salida dist, cuatro variables VITE_ |
...
- VITE_TURNSTILE_SITE_KEY: clave pública del CAPTCHA. Su secreto se configura en Supabase Auth.

Solo en tu computadora para crear el administrador inicial:

- SUPABASE_SECRET_KEY (solo .env.admin).
...
3. Crear admin si aún no existe; para una actualización no repetir ese paso. Activar y verificar su segundo factor.
4. Subir a GitHub y desplegar en Vercel con las cuatro variables públicas.
...
- Las pruebas reales de Auth, Storage y envío de correo siguen pendientes en tus servicios. La compilación y pruebas locales no garantizan todos esos servicios.
...
La guía principal actualizada para esta versión es INSTRUCCIONES.md; revisa también SECURITY.md.
